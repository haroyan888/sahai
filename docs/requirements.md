...
REST API(`/api/*`)の全エンドポイント・リクエスト/レスポンス形式・エラーコードは [api-design.md](./api-design.md) を正とする。CLIの `container push` はビルド元マシンで直接 `docker build`/`docker push` するのみでAPIを経由しないため、APIのスコープは「登録済みサービスの管理」と「Control plane自身の設定」に限られる。唯一の例外がログ配信のエンドポイントで、これだけはJSONを1往復で返さずServer-Sent Eventsで流し続ける(9章参照)。
...
falseにすると80番から平文httpのまま直接サービスへアクセスできる。このとき443番も同じ内容で応答する(自己署名証明書。ACME証明書の取得は試みない)。`SAHAI_DOMAIN`を`localhost`等にしたテスト環境ではLet's Encrypt証明書を取得できず自己署名証明書のままになるため、falseにするとよい。

**`SAHAI_HTTPS_REDIRECT`がtrueでもfalseでも、httpとhttpsで見えるものは同じでなければならない**(trueならhttpが301でhttpsへ寄せられ、falseなら両方が同じ内容を直接返す)。Traefikでは`tls`キーを持つルーターがwebsecure専用、持たないルーターがweb専用になり、単一ルーターで両プロトコルに応答させられないため、falseのときは同じルールのルーターをweb用・websecure用の2本書き出して揃える。プロトコルによって管理画面に入れたり入れなかったり、未登録サブドメインの案内ページが出たり404になったりすると、原因の切り分けが著しく難しくなるため。
...
案内ページはエンドユーザーが見る画面なので、**どの状態でも`h1`に何が起きたか(エラー名)、`p`にその詳細**、という同じ構成にする。管理画面のヘッダー(サービス一覧へのリンク・ログアウト)は表示せず、カード1枚を画面中央に置くだけのレイアウトにする。
...
#### Not Serviceページへの誘導(Hostヘッダーによる振り分け)

Not ServiceページはSPAの`/not-service`というパスにしか存在しないため、転送されてきたリクエスト(通常は`/`)をそのままSPAへ渡すだけでは到達できない。SPAは`/`で起動して認証ゲートに落ち、**エンドユーザーには身に覚えのないログイン画面が出てしまう**。外部公開するサービスがある以上これは許容できないため、Control planeはSPAのエントリポイント(`index.html`)を返す直前にHostヘッダーを見て振り分ける:

- ホスト名が`sahai.<ベースドメイン>`(管理画面)である → 従来どおりSPAを返す
- ホスト名がベースドメインの**それ以外のサブドメイン**である → `/not-service`へリダイレクトする
- 上記以外(`localhost`・生IP・ベースドメイン外のホスト名)、およびベースドメインが未確定な初期設定前 → 従来どおりSPAを返す

3つ目の条件により、Traefikを経由しない直接アクセスや初期設定フローが巻き込まれることはない(catch-allルートも`*.<ベースドメイン>`にしかマッチしないため、判定範囲は一致する)。

この振り分けは**SPAフォールバック(ディスク上に実ファイルが無いパス)でのみ行う**。JS/CSS等のアセットは実ファイルとして先に配信されるため、リダイレクト対象にならない(アセット取得までリダイレクトするとNot Serviceページ自体が描画できなくなる)。
...
**システム側でログの収集・保存は行わない**。Dockerが既に保持しているものを読み出して中継するだけで、Control plane側では一切永続化しない。保持期間・容量の上限はDockerのログドライバの設定に従う。

- **閲覧先はWeb UIのサービス詳細画面のみ**。CLIからは提供しない(ホスト上で作業できるなら`docker logs svc-<ServiceContainer.id>`を直接叩くほうが早いため)
- 標準出力・標準エラーの両方を、発生順のまま1つの流れとして表示する。どちらから出たものかは行ごとに区別できるようにする
- **接続した時点で直近の一定行数を送り、以降は追記されるたびに流す**(`docker logs --tail N --follow`相当)。画面を開いている間だけ流れ、閉じれば止まる
- 対象は`ServiceContainer`単位。サービスが複数コンテナを持つ場合(compose型)は画面上で切り替える。image型・compose型のどちらもコンテナ名は`svc-<ServiceContainer.id>`に揃えてあるため(7章)、取得方法は同一
- **停止したサービスの過去のログは読めない**。差配はstopでコンテナごと削除するため(7章)、Dockerが持っていたログも一緒に消える。画面にはその旨を出す。なお起動失敗の理由は`last_error`に保存済みで、そちらはサービス詳細に表示される

**方式はServer-Sent Events**(`GET /api/services/{id_or_name}/logs`)。ヘルスチェック・リソース監視はポーリングで済ませているが(8章・10章)、ログは行の発生が不規則で、ポーリング間隔がそのまま表示の遅れになるうえ、間隔を詰めると空振りが増える。WebSocketは使わない — 必要なのはサーバーからの一方向の送信だけで、双方向通信の機構は要らないため。

**認証はAuthorizationヘッダーのままとする**。ブラウザの`EventSource`はヘッダーを付けられないが、そのためにトークンをクエリ文字列へ逃がすことはしない(URLはアクセスログ・プロキシ・ブラウザ履歴に残るため)。Web UI側は`fetch`のストリームを読んでSSEを自前で解釈する。
